...
class SimpleMessageCheckerTest {
    ...
    void shouldNotAllowEmptyMessageForFailedTests() {
        final CheckerException e = assertThrows(CheckerException.class, () -> new SimpleMessageChecker("someName", false, null).check());
        assertThat(e).hasMessageThat().contains("Feedback for non-successful checkes cannot be null or empty!");

        final CheckerException e2 = assertThrows(CheckerException.class, () -> new SimpleMessageChecker("someName", false, "\t\n   ").check());
    ...
    void shouldNotAllowEmptyTestNames() {
        final CheckerException e1 = assertThrows(CheckerException.class, () -> new SimpleMessageChecker("", false, "asd").check());
        assertThat(e1).hasMessageThat().contains("CheckerName cannot be null or empty!");

        final CheckerException e2 = assertThrows(CheckerException.class, () -> new SimpleMessageChecker("  \t ", true, "x").check());
        assertThat(e2).hasMessageThat().contains("CheckerName cannot be null or empty!");

        final CheckerException e3 = assertThrows(CheckerException.class, () -> new SimpleMessageChecker(null, false, "d8").check());
